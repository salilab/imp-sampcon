...
class Tests(unittest.TestCase):
    def test_exhaust(self):
        ...
        make_models()
        gsm_dir = os.path.join(TESTDIR, 'modeling', 'good_scoring_models')
        script = utils.get_script(TOPDIR,
        ...
        expected = [
            ...
            'Identities_B.txt', 'cluster.0.all.txt', 'cluster.0.sample_A.txt',
            ...
            'test.Top_Score_Conv.txt', 'cluster.0/cluster_center_model.rmf3',
            'cluster.0/LPD_TestAll.mrc', 'cluster.0/Sample_A/LPD_TestAll.mrc',
            'cluster.0/Sample_B/LPD_TestAll.mrc']

        for e in expected:
            os.unlink(os.path.join(TESTDIR, e))
        os.rmdir(os.path.join(TESTDIR, 'cluster.0', 'Sample_A'))
        os.rmdir(os.path.join(TESTDIR, 'cluster.0', 'Sample_B'))
        os.rmdir(os.path.join(TESTDIR, 'cluster.0'))
        shutil.rmtree(os.path.join(TESTDIR, 'modeling', 'good_scoring_models'))
