...
stats sprintf("%s.CP.txt", ARG1) usi 1 prefix "A"
minx = 0
maxx = A_max + 1.75
set xr [minx:maxx]
...
stats sprintf("%s.CP.txt", ARG1) usi 2 prefix "N1"
stats sprintf("%s.CP.txt", ARG1) usi 3 prefix "N2"
...
stats sprintf("%s.PV.txt", ARG1) usi 2 prefix "B2"
stats sprintf("%s.PV.txt", ARG1) usi 3 prefix "B3"
...
set output sprintf("%s.CP.pdf", ARG1)
plot sprintf("%s.CP.txt", ARG1) usi ($1+0.5):2:xtic(sprintf("Cluster %i", $1)) w boxes notitle lc rgb "#EB7262", \
